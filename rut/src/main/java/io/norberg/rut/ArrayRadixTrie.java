...
final class ArrayRadixTrie<T> {

  private static final byte SLASH = '/';
  private static final byte QUERY = '?';

  private static final int SIBLING = 1 << 31;
  private static final int EDGE = 1 << 30;
  private static final int VALUE = 1 << 29;
  private static final int LENGTH = ~(SIBLING | EDGE | VALUE);
  private static final int CAPTURE = LENGTH;

  /**
   * 0: HEADER (SIBLING? | VALUE? | EDGE? | LENGTH)
   * 1: SIBLING_INDEX
   * 2..2+CEIL(LENGTH / 4.0): DATA
   * 3+LENGTH: VALUE_INDEX
   * 4+LENGTH: EDGE_INDEX
   */
  ...
  public ArrayRadixTrie(final int[] nodes, final List<T> values, final int captures) {
    this.values = values;
    this.nodes = nodes;
  ...
  T lookup(final CharSequence path, @Nullable final Captor captor) {
    ...
    return lookup(0, path, 0, captor, 0);
  }

  private T lookup(final int node, final CharSequence path, final int i,
    ...
    while (true) {
      T value;
      final int length = length(node);
      if (length == CAPTURE) {
        value = capture(node, path, i, captor, capture);
      } else {
        value = match(node, length, path, i, captor, capture);
      }
      if (value != null) {
        return value;
      }
  ...
  private int length(final int node) {
    return nodes[node] & LENGTH;
  }

  private boolean hasSibling(final int node) {
    return (nodes[node] & SIBLING) != 0;
  }

  private T match(final int node, final int length, final CharSequence path, final int index,
                  @Nullable final Captor captor, final int capture) {
    if (index + length > path.length()) {
      return null;
    }

    int data = hasSibling(node) ? node + 2 : node + 1;
    ...
    while (i < length) {
      int chars = nodes[data];
      for (int j = 0; j < 4 && i < length; j++, i++, chars >>= 8) {
        c = path.charAt(index + i);
        final byte p = (byte) (chars & 0xFF);
        if (c != p) {
          return null;
        }
      }
  ...
  private T value(final int node, final int length) {
    if ((nodes[node] & VALUE) == 0) {
      return null;
    } else {
      final int sibling
      final int i = hasSibling(node) ?
    }
  }

  private T capture(final int node, final CharSequence path, final int index,
                    @Nullable final Captor captor,
                    final int capture) {
    int i;
    char c = 0;
